...
async function acquireLeaderLock() {
    const key = intEnv('WORKER_LEADER_LOCK_KEY', 931337);

    // We must keep ONE client checked out for the lifetime of the worker
...
async function main() {
    ...
    // Optional: if you want the “one circulating dispatch job” behavior
    ...
    while (!shuttingDown) {
        try {
            // enqueue refresh jobs for active searches (your existing function)
